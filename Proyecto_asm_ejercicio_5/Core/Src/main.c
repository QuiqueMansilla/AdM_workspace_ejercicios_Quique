...
void filtroVentana10(uint16_t * vectorIn, uint16_t * vectorOut, uint32_t longitudVectorIn)
{
	// uint32_t longitudVectorIn = 10; //cantidadde muestras del vector de entrada - al de salida
	// uint16_t * vectorIn = {20, 25, 28, 30, 28, 25, 20, 17, 15, 10}; //contiene 10 muestras
	// uint16_t * vectorOut[longitudVectorIn];
	uint16_t L = 9; //Cantidad de muestras de la ventana
	uint16_t N = 10; //longitudVectorIn

	//Valor de vectorOut en el extremo final del vectorIn
	vectorOut[N-1] = 0;

	for (uint16_t i = 0; i > 5; i++)
	{
		vectorOut[N-1] = vectorOut[N-1] + vectorIn[N-i];
	}

	vectorOut[N-1] = vectorOut[N-1]/L;

	//vectorOut[N-1]=(vectorIn[N-1]+ vectorIn[N-2]+ ... + vectorIn[N-5])/L
}
